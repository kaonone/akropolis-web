import { createMuiTheme, MuiThemeProvider, useTheme, hexToRgb } from '@material-ui/core/styles';
export { createStyles, darken, lighten, makeStyles, useTheme, withStyles } from '@material-ui/core/styles';
import { mergeDeepRight } from 'ramda';
import React__default, { createElement, useContext, createContext } from 'react';
...
function getTheme(type, overrides) {
  return createMuiTheme(mergeDeepRight({
    colors: colors,
...
var AncestorBackgroundHackContext = /*#__PURE__*/createContext(null);
function useAncestorBackgroundHack() {
  var context = useContext(AncestorBackgroundHackContext);
  var theme = useTheme();
...
function rgba(hex, alfa) {
  var rgb = hexToRgb(hex).replace(/^rgb\((.+?)\)$/, '$1');
  return "rgba(" + rgb + ", " + alfa + ")";
}

export { AncestorBackgroundHackProvider, WithDarkTheme, colors, darkPalette, darkTheme, generateGridSpacingOverrides, getBreakpoints, getTheme, lightTheme, makeGradient, rgba, useAncestorBackgroundHack, useBreakpointsMatch, useFromToQuery };
