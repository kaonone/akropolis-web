...
export function InactiveDelphiPoolIcon(props: React.ComponentProps<typeof SvgIcon>) {
  const svgIds = React.useMemo(() => [1, 2, 3].map(() => makeRandomID('DelphiPoolIcon')), []);

  return (
    <SvgIcon {...props} viewBox="0 0 20 20">
      <defs>
        <linearGradient id={svgIds[0]} x1="50%" x2="50%" y1="0%" y2="100%">
      ...
      <g fill="none" fillRule="evenodd">
        ...
        />
        <mask id={svgIds[2]} fill="#fff">
          <use xlinkHref={`#${svgIds[1]}`} />
        </mask>
        <use fill={`url(#${svgIds[0]})`} xlinkHref={`#${svgIds[1]}`} />
        <path
          fill="#FFF"
          stroke="#0A0A0E"
          strokeWidth=".5"
